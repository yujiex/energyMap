...
definition include: National Heat Map that visualizes heat demand
density of different building sectors, using GIS as a decision support
to for assessing Renewable Energy Potential~\cite{Voivontas1998333},
evaluating different choices of power plant~\cite{Yeo201499},
evaluation district system expanssion~\cite{Finney2012165}.

\subsubsection{Coupled Geo-database and Energy Simulation Platform}
Even further, the new development of Dynamic Energy Map would provide
stooth connections to urban level energy simulation tools support the
performance-based community energy planning and geo-design.
...
load. The building operation schedule vary greatly from building to
building as a result of difference in building type and occupant
behavior. Different operation scheduce indicates a non-coherent
arrival of peak demand within a mixed-use urban
environment. Difference in building type also suggest difference in
indoor environment parameter requirement such as ventilation rate,
lighting intencity etc., indicating a dramatic variation in energy
consumption data distribution between buildings. Upon these
considerations, a simple annual or monthly average cannot effectly
represent the real energy consumption behavior of each building and
the whole urban environment. In order to present this complecated
behavior of time-depandent energy demand, the time dimension is
necessary.
...
at each given time spot or time window according to the design context.
\newpage
...
\subsubsection{Support District System Design}
...
water~\cite{baird2014}. The district system helps reducing
environmental impact by harvesting residual energy in the form of
rejected heat or coolth in the process of electricity generation. It
is also more efficient in energy generation , and by using waste in energy generation. Other
non-environmental benifits include reducing the space dedicated to
mechanical system and improve design flexibility in buildings and less
harmful gas production from stand-alone boiler conbustion.

Dynamic Energy Map support district energy system design by 1)
revealing the non-coherant peak demand of heat and electricity, so
that the size of the co-generation system capacity is not the sum of
the peak demand of each individual building within the system
coverage. 2) Support comparison of aggregate energy demand for
different design choices so that the design alternative with more
stable demand could stand out among all design alternatives.

\subsubsection{Support Community Energy Management}
Community Energy Management is a combination of urban planning,
transportation management and infrastructure design.

\subsubsection{Support Geo-design}
\subsubsection{Definition of Dynamic Energy Map according to the use
  cases above}
According to the use cases above we selected the heating and cooling
demand as the major variable in discussion.
We define ``Local'' Dynamic Energy Map for Community level design support as:
\begin{itemize}
...
\item An interface that has multi-dimensional graphical display of the
  ...
  \item 1D: data plot for providing quantitative information 
    ...
    demand of the whole community and major building sectors
    throughout the year. It enables comparison of different urban
    design alternatives in terms of total demand and demand
    variation. These information supports district system planning by
    optimizing the load of buildings in a mix-used community so that
    they have a stabler demand profile.

  \item 2D/3D: graphical display of spatial relationship of energy
    data

    The Local Dynamic Energy Map applies a graduated symbol or color
    to buildings in the community aiming at providing the intuition of
    the building energy demand behavior within a community. It helps
    identify the rank of energy demand in a community and provides a
    guidance in energy cascading design.
    
    For univariant map senario, we suggest the variant size
    symbol. For bivariant map senario, such as the case for the
    current study, we suggest a two dimentional color ramp.
