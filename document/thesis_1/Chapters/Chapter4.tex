...
\begin{figure}[h!]
  ...
  \centering
  \includegraphics[width=\linewidth]{recover2d}
...
\begin{subfigure}{0.7\textwidth}
  \centering
  \includegraphics[width=\linewidth]{recover3d}
...
transmission loss (\fref{fig:recoverGr}).

\begin{figure}[h!]
  \centering
  \begin{subfigure}{0.7\textwidth}
  \centering
  \includegraphics[width=\linewidth]{recoverGr2d}
...
\begin{subfigure}{0.7\textwidth}
  \centering
  \includegraphics[width=\linewidth]{recoverGr3d}
...
much heat recovery one could achieve: per \eref{eq:recover} and the
analysis in \sref{sec:inputRecover}, the heat recovery potential of
...
\item Identify anchor load building
  ...
  Although with the box plot of heating demand (\fref{fig:H}), the
...
\item Local load balancing
  ...
  combination'' (\fref{fig:plotAll}) at this time that could be a
  ...
  output a graph of aggregated load (\fref{fig:seeAggDemand}).
...
\begin{itemize}
\item Provide both 2D and 3D map display as a result of the debating
  situation mentioned in \sref{2d3d}.
\item Choose bivariate choropleth map representation which has the
  highest accuracy rate in map reading experiments \sref{symbolChosen}.
\item Providing the most commonly used data classification method:
  Equal Interval and Quantile Interval \sref{dataClassification}
\item The number of classes for energy data classification is chosen
  to be 7, less than the suggested value of 10 as a result of smaller
  display size than 1024x768 \sref{symbolChosen}
\item Provide both linear and periodical navigation based on section
  \sref{timeRepresent}
\item Noise removal in map display of energy profile
  data~\cite{Dorling1992}. This is done through the discrete color
...
\begin{figure}[h!]
  ...
  \begin{subfigure}{0.7\textwidth}
  \centering
  \includegraphics[width=\linewidth]{interface0720}
...
\begin{subfigure}{0.7\textwidth}
  \centering
  \includegraphics[width=\linewidth]{interface2d}
